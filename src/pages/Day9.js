...
class Day9 extends React.Component {
    constructor(props) {
        ...
        this.aboutStyle = { backgroundColor: "#222", padding: 30, color: "white", margin: '0 auto', marginTop: -100, width: '50%', borderRadius: 5 }
    }
    ...
    getNavList = () => {
        ...
        return list.map((x, i) => {
            return (
                <li i={i} className="navList" style={{ fontSize: '1.3em', borderBottom: activeList == i ? "2px solid #E0C3FC" : "2px solid transparent" }} onClick={() => this.handleToggle(i)} >{x}</li>)
        })
    }
    getAbout = () => {
        return (
